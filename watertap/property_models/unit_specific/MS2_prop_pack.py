...
#################################################################################
"""
Initial property package for H2O-MS2 system
"""
...
@declare_process_block_class("MS2ParameterBlock")
class MS2ParameterData(PhysicalParameterBlock):
    ...
    def build(self):
        ...
        """
        super(MS2ParameterData, self).build()

        self._state_block_class = MS2StateBlock
        ...
        # molecular weight
        mw_comp_data = {"H2O": 18.01528e-3, "MS2": 3600}
        self.mw_comp = Param(
            self.component_list,
...
class _MS2StateBlock(StateBlock):
    """
...
@declare_process_block_class("MS2StateBlock", block_class=_MS2StateBlock)
class MS2StateBlockData(StateBlockData):
    def build(self):
        """Callable method for Block construction."""
        super(MS2StateBlockData, self).build()
        ...
        self.flow_mass_phase_comp = Var(
            ...
            initialize={("Liq", "H2O"): 0.999999926, ("Liq", "MS2"): 74e-9},
            bounds=(1e-18, None),
    ...
    def _molality_phase_comp(self):
        ...
        self.eq_molality_phase_comp = Constraint(
            self.params.phase_list, ["MS2"], rule=rule_molality_phase_comp
        )
    ...
    def calculate_scaling_factors(self):
        ...
        if iscale.get_scaling_factor(self.flow_mass_phase_comp["Liq", "MS2"]) is None:
            sf = iscale.get_scaling_factor(
                self.flow_mass_phase_comp["Liq", "MS2"], default=1e8, warning=True
            )
            iscale.set_scaling_factor(self.flow_mass_phase_comp["Liq", "MS2"], sf)
